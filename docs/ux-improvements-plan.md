...
FORMAT REQUIREMENTS:
- Use compact bullet points (•) not numbered lists
- One rule per line
...
## File Organization
• Components: src/components/{FeatureName}/index.tsx
• Tests: src/components/{FeatureName}/__tests__/index.test.tsx
...
EXAMPLE OF CONCISE FORMAT:
## Dependency Rules
• UI components can import: hooks, utils, types
• UI components CANNOT import: services, API clients
• Services can import: utils, types, API clients
...
- coding-standards.md: ~100-125 lines (6 sections × ~20 lines)
- architecture-rules.md: ~100-125 lines (7 sections × ~18 lines)
...
### Phase 1 Testing
- [ ] Fresh `taskflow init` → `taskflow prd create` → `taskflow prd generate-arch` works
...
## 🔥 ADDITIONAL CRITICAL ISSUES (2026-01-02 Update)

After user testing, **3 more critical issues** were identified:

### **Issue #8: No AI Context/Token Usage Display** 🔴 **HIGH**
...
### **Issue #9: No Streaming Output from AI** 🔴 **CRITICAL**
...
- Only supports `generate()` → complete response
...
### **Issue #10: Task Generation Creates Too Many Small Tasks** 🟡 **MEDIUM**
...
### **Issue #11: tasks-progress.json Redundancy?** ✅ **NOT AN ISSUE**
...
```
Data Structure:
├── project-index.json (features list only)
├── tasks-progress.json (full hierarchy, task refs only)
├── F*/F*.json (feature file with stories/task refs)
└── F*/S*/T*.json (individual task details)
```
...
**Conclusion**: ✅ **This is good architecture** - NOT redundant
...
## 📅 Phase 6: AI Context & Token Tracking (NEW - Priority: HIGH)
...
## 📅 Phase 7: Streaming AI Responses (NEW - Priority: CRITICAL)
...
## 📅 Phase 8: Optimize Task Generation Granularity (NEW - Priority: MEDIUM)
...
## 🔥 CRITICAL DESIGN FLAW: Tech Stack Unknown (2026-01-02 Update #2)

### **Issue #12: Cannot Generate Meaningful Architecture Without Tech Stack** 🔴 **CRITICAL**
...
3. ❌ AI doesn't know tech stack
4. ❌ AI guesses or hallucinates generic rules
...
## 📅 Phase 9: Interactive Tech Stack Selection (NEW - Priority: CRITICAL)
...
export class PrdGenerateArchCommand extends BaseCommand {
  async execute(prdFile: string): Promise<CommandResult> {
    ...
    if (!isGreenfield && Object.keys(detectedStack).some(k => detectedStack[k]?.length > 0)) {
      ...
      console.log('✓ Detected existing tech stack:\n');
    ...
    options.forEach((option, i) => {
      const marker = option.recommended ? chalk.green('✓ RECOMMENDED') : '';
      ...
      console.log('\n    Pros:');
      option.pros.forEach(pro => console.log(chalk.green(`      ✓ ${pro}`)));

      console.log('\n    Cons:');
      option.cons.forEach(con => console.log(chalk.yellow(`      ⚠ ${con}`)));

      console.log('\n    Best for:');
      option.bestFor.forEach(use => console.log(chalk.dim(`      • ${use}`)));
    ...
    console.log(chalk.green(`✓ Generated tech-stack.md (${techStackContent.split('\n').length} lines)`));
...
❌ moment
   ...
   ⚠️  DEPRECATED: This package is in maintenance mode
   💡 Use date-fns or dayjs instead

⚠️  request
   Latest: 2.88.2
   ⚠️  Low usage: Only 234 downloads/week - may not be maintained
```
  ...
  for (const option of options) {
    ...
    for (const result of results) {
      if (result.validated.isDeprecated) {
        ...
        option.cons.unshift(`⚠️  Contains deprecated: ${result.package}`);

        if (result.recommendation) {
          option.cons.push(`💡 ${result.recommendation}`);
        }
      }
...
🔍 Validating package versions and checking for deprecations...
...
async enhanceWithWebSearch(options: TechStackOption[]): Promise<TechStackOption[]> {
  ...
  console.log('🌐 Searching for current best practices...\n');
...
🔗 Checking dependency compatibility...
   ...
   ❌ next@13.5.0 requires react@^18.2.0
      Selected: react@19.0.0
      CONFLICT: React 19 not supported by Next.js 13

   🔧 Auto-adjusting versions for compatibility...
      next: 13.5.0 → 14.1.0 (supports React 19)
...
─────────────────────────────────────────────────────────────
next            | 14.1.0  | react: ^18.2.0 or ^19   | ✅
react           | 19.0.0  | -                       | ✅
react-dom       | 19.0.0  | react: 19.0.0 (exact)   | ✅
tailwindcss     | 3.4.1   | postcss: ^8.0.0         | ✅
```
...
private async validateOptions(options: TechStackOption[]): Promise<TechStackOption[]> {
  for (const option of options) {
    ...
    if (!compatResult.compatible) {
      console.log(`\n   ⚠️  Compatibility issues in "${option.name}":\n`);
      ...
      if (compatResult.adjustedVersions) {
        console.log('\n   🔧 Auto-adjusting versions...\n');
        compatResult.adjustedVersions.forEach((version, pkg) => {
          packageVersionMap.set(pkg, version);
          console.log(`      ${pkg}: ${version}`);
        });
        option.cons.push('⚠️  Some versions were adjusted for compatibility');
      } else {
        // Can't auto-fix
        option.cons.unshift('❌ Has unresolvable compatibility issues');
        option.recommended = false;
      }
    } else {
      console.log(`   ✅ All packages compatible in "${option.name}"\n`);
    }
  }
...
|---------|--------|----------|-------|
| next | 14.1.0 | 14.1.0 | ✅ Latest stable |
...
| next | 14.1.0 | react: ^18.2.0 or ^19.0.0 | ✅ |
| react | 18.2.0 | - | ✅ |
| react-dom | 18.2.0 | react: 18.2.0 (exact) | ✅ |
| tailwindcss | 3.4.1 | postcss: ^8.0.0 | ✅ |
...
### Critical Exact Matches
- React + React DOM: 18.2.0 = 18.2.0 ✅
- @types/react major: 18.x matches React 18.x ✅

### Compatibility Requirements
- Next.js 14: Supports React 18.2+ or React 19
- Tailwind CSS 3: Requires PostCSS 8.x (have 8.4.33 ✅)

### Upgrade Path
- React 18 → 19: Requires Next.js 14.1+ (we have 14.1.0 ✅)
